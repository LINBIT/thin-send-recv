...
RELEASE = $(shell sed -ne '/^Release:/{s/Release: \(.*\)/\1/;p;q;}' thin_send_recv.spec)
...
tgz: $(all-src)
	tar --transform="flags=rSh;s,^,thin-send-recv-$(VERSION)-$(RELEASE)/," \
		--owner=0 --group=0 -czf - $(all-src) > thin-send-recv-$(VERSION)-$(RELEASE).tar.gz
